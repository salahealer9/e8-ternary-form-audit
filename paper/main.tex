...
\Omega(X,Y,Z)=B(X,[Y,Z]),
\]
where $B$ is an invariant nondegenerate bilinear form.  We give an exact sparse realization of this form for $\mathfrak e_8$ in a fixed $\epsilon$-canonical Chevalley basis and a fixed normalization $B_0$ of the invariant form.  In this basis the complete ordered domain contains $248^3=15{,}252{,}992$ triples, of which exactly $16{,}176$ have nonzero coefficient.  These split into $13{,}440$ root--root--root coefficients and $2{,}736$ Cartan--root--root coefficients.  The complete nonzero coefficient spectrum is
\[
-2:24,\qquad -1:8064,\qquad +1:8064,\qquad +2:24.
\]
The root-sector support is identified with the zero-sum ternary incidence relation on the $240$ roots of $E_8$:
\[
\Omega(e_\alpha,e_\beta,e_\gamma)\neq 0
\quad\Longleftrightarrow\quad
\alpha+\beta+\gamma=0.
\]
This gives a $240$-vertex, $2240$-edge linear $3$-uniform hypergraph representation of the root support.  We derive the headline support counts analytically, construct the full signed sparse tensor by two algorithmically distinct exact routes, and validate the result by an independent reconstruction of the $248$-dimensional Lie bracket and invariant form.  The Jacobi identity is checked on all $\binom{248}{3}=2{,}511{,}496$ distinct basis triples, and the previously constructed sparse tensor is recovered entry-for-entry from $B_0(X,[Y,Z])$.  Finally, raising the first index of the sparse tensor with $B_0^{-1}$ recovers all $248^2=61{,}504$ ordered basis brackets exactly.  The mathematical structures used are classical; the contribution is an explicit sparse computational realization, complete coefficient census, root-incidence packaging, and reproducible validation architecture.
\end{abstract}

\section{Introduction}

The exceptional Lie algebra $\e_8$ is simultaneously one of the most rigid and one of the most computationally demanding finite-dimensional simple Lie algebras.  Its rank is $8$, its dimension is $248$, and its root system contains $240$ roots.  Exact computational models of $\e_8$ have a long history: Chevalley bases reduce the structure constants to integers, explicit $E_8$ commutation data have been tabulated, and modern formulas provide canonical structure constants in simply-laced types, including $E_8$ \cite{Djokovic2000,GeckLang2025}.  Explicit formulas for the $E_8$ bracket also exist in alternative realizations \cite{Kollross2025}, while recent public software provides sparse machine-readable structure constants \cite{McGirl2026}.

A different but closely related object is the Cartan 3-form
\begin{equation}\label{eq:cartan3}
\Omega(X,Y,Z)=B(X,[Y,Z]),
\end{equation}
where $B$ is an invariant nondegenerate symmetric bilinear form.  Cartan 3-forms are classical objects attached to simple Lie algebras and have been studied geometrically and multisymplectically; see, for example, \cite{Le2013}.  In physics, the same object appears as the fully lowered, completely antisymmetric structure tensor; explicit $E_8$ formulas in a different basis are standard in supergravity and related literature \cite{KoepsellNicolaiSamtleben1999}.

The purpose of the present paper is therefore not to introduce a new invariant.  Instead, we ask for a complete exact sparse realization of the normalized $E_8$ Cartan 3-form in a fixed $\epsilon$-canonical Chevalley basis, together with a transparent combinatorial description of its support and an independent validation chain.  The construction is designed so that the main numerical statements can be derived analytically rather than inferred only from computer output.

Our main results are as follows.  First, on root vectors the support of the Cartan 3-form is precisely the zero-sum ternary relation
...
Second, after including the Cartan directions, the complete sparse support has
\begin{equation}\label{eq:16176intro}
13{,}440+2{,}736=16{,}176
\end{equation}
ordered nonzero coefficients.  In the basis and normalization fixed below, the coefficient multiplicities are
\begin{equation}\label{eq:spectrumintro}
-2:24,\qquad -1:8064,\qquad +1:8064,\qquad +2:24.
\end{equation}
The counts in \eqref{eq:16176intro} and \eqref{eq:spectrumintro} admit short analytic derivations from the standard $E_8$ root inner-product census and alternation.

Third, we construct the full signed sparse tensor by two algorithmically distinct exact routes: directly from ordered Lie brackets and invariant pairings, and from canonical unordered root triples/opposite-root lines followed by alternating extension.  The two sparse coefficient maps agree exactly.

Finally, we reconstruct the bracket and normalized bilinear form independently of the tensor and validate the algebra exhaustively.  The Jacobi identity is checked on all
\[
\binom{248}{3}=2{,}511{,}496
\]
distinct basis triples.  We then reconstruct $B_0(X,[Y,Z])$ from this independently validated algebra and recover the sealed sparse tensor entry-for-entry.  Conversely, because $B_0$ is nondegenerate, the bracket is recovered from the trilinear tensor by raising its first index; this equality is verified on all $248^2=61{,}504$ ordered basis pairs.

The paper is organized as follows.  Section~\ref{sec:conventions} fixes the $E_8$ root, basis, sign, and bilinear-form conventions.  Section~\ref{sec:incidence} develops the zero-sum ternary incidence structure.  Section~\ref{sec:sparse} constructs and counts the sparse Cartan 3-form.  Section~\ref{sec:validation} describes the two-route construction and independent algebraic validation.  Section~\ref{sec:data} specifies the machine-readable realization.  Section~\ref{sec:priorart} states the prior-art and novelty boundary explicitly, and Section~\ref{sec:discussion} discusses computational consequences.
...
\end{equation}
where the roots are sorted lexicographically in doubled coordinates.  This fixes every coefficient reported below.
...
The $1120$ count is the familiar number of $A_2$ root subsystems of $E_8$; it is listed explicitly by Bordner, Manton, and Sasaki \cite{BordnerMantonSasaki2000} and also derived directly from the $240\cdot56$ root-pair census in \cite{BohningBothmerMarquand2026}.  We use the zero-sum representation because it exposes the ternary support directly.
...
\end{center}
A public archival release of the source code and deterministic data artifact will accompany the final version of the manuscript; the release URL/DOI will be inserted here.
