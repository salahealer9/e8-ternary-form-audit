...
Corresponding author: \texttt{salahealer@gmail.com}}
\date{}
...
\begin{abstract}
We give an exact sparse realization of the normalized Cartan 3-form \(g(X,Y,Z)=B_0(X,[Y,Z])\) for the Chevalley \(\mathbb Q\)-form of \(E_8\) in a fixed \(\epsilon\)-canonical Chevalley basis. Among the \(248^3=15{,}252{,}992\) ordered basis triples, exactly \(16{,}176\) have nonzero coefficient: \(13{,}440\) in the root--root--root sector and \(2{,}736\) in the Cartan--root--root sector. The complete nonzero coefficient census is \(-2:24,-1:8064,+1:8064,+2:24\). On root vectors, support is exactly the zero-sum relation \(\alpha+\beta+\gamma=0\), giving a 240-vertex, 2240-edge linear 3-uniform hypergraph. We derive the headline counts analytically, construct the signed tensor by two algorithmically distinct exact routes, and validate it against an independently reconstructed \(E_8\) bracket and invariant form. Exhaustive Jacobi verification covers all \(2{,}511{,}496\) distinct basis triples, and raising the first tensor index recovers all \(61{,}504\) ordered basis brackets exactly. The underlying Lie-theoretic structures are classical; the contribution is the explicit sparse computational realization, coefficient census, deterministic machine-readable representation, and reproducible validation architecture.
\end{abstract}

\noindent\textbf{Keywords:}
$E_8$ Lie algebra; Cartan 3-form; Chevalley basis; root systems;
computational algebra; sparse tensors; reproducible mathematics.

\noindent\textbf{2020 Mathematics Subject Classification:}
17B25; 17B20; 68W30.

\noindent\textbf{Word count:} 3,495 (excluding references).
...
The paper is organized as follows.  Section~\ref{sec:methods} fixes the $E_8$ root, basis, sign, and bilinear-form conventions and the exact construction framework.  Section~\ref{sec:results} presents the zero-sum incidence structure, complete sparse Cartan 3-form, exact validation, and machine-readable realization.  Section~\ref{sec:discussion} places the construction against prior art, discusses its computational interpretation and scope, and concludes.

\section{Materials and methods}\label{sec:methods}

\subsection{Conventions and the \texorpdfstring{$\epsilon$-canonical $E_8$}{epsilon-canonical E8} basis}\label{sec:conventions}
...
\subsubsection{Simple roots and Cartan matrix}
...
\subsubsection{\texorpdfstring{$\epsilon$-canonical}{epsilon-canonical} Chevalley signs and invariant form}
...
\subsection{Zero-sum root incidence}\label{sec:incidence}

\subsubsection{Root support of the Cartan 3-form}
...
\subsubsection{A \texorpdfstring{$3$}{3}-uniform root hypergraph}
...
\subsection{The complete sparse Cartan 3-form}\label{sec:sparse}
...
\subsubsection{Coefficient multiplicities}
...
\subsection{Independent construction and validation}\label{sec:validation}

\subsubsection{Two algorithmically distinct exact tensor constructions}
...
The resulting sparse maps contain $16{,}176$ entries each and agree exactly: there are zero direct-only keys, zero reconstruction-only keys, and zero shared keys with unequal coefficients.  This is stronger than agreement of aggregate counts: support and signed coefficients coincide entry-for-entry.

\subsubsection{Independent reconstruction of the Lie algebra}
...
\subsubsection{Recovering the bracket from \texorpdfstring{$g$}{g}}
...
\subsection{Machine-readable realization and reproducibility}\label{sec:data}
...
\subsection{Prior art and contribution boundary}\label{sec:priorart}
...
\subsection{Computational interpretation and scope}\label{sec:discussion-scope}
...
No funding was obtained for the research reported in this article or for the preparation of the manuscript.

\section*{Data availability statement}

The exact machine-readable data supporting the results of this study are publicly archived with the frozen reproducibility release on Zenodo \cite{Gherbi2026E8Audit}, at \url{https://doi.org/10.5281/zenodo.22676423}.  The canonical sparse Cartan 3-form artifact is \nolinkurl{data/derived/phase3d_signed_sparse_cartan_form.json}, with SHA-256 \nolinkurl{43b52b4116dfcf0dace332234b5c9dc50d95d51effb690aec8aebe39ad4f92e8}.

\section*{Software and code availability}

The complete source code, deterministic phase runners, automated validation suite, and reproducibility records are publicly available at \url{https://github.com/salahealer9/e8-ternary-form-audit}.  The frozen reproducibility release \cite{Gherbi2026E8Audit} is \texttt{v1.0.0}, commit \nolinkurl{2000d93478be897f0742191aafd173d71e481fbd}, archived on Zenodo at \url{https://doi.org/10.5281/zenodo.22676423}.

\section*{Author contributions}

S.-E.G.\ is the sole author and was responsible for conceptualization, methodology, software, formal analysis, validation, investigation, data curation, and writing and review of the manuscript.

\section*{Declaration of generative AI use}

The author used OpenAI ChatGPT (GPT-5.6 Sol) during the research and manuscript-preparation workflow for idea exploration, software coding assistance, computational organization, diagnostic review, consistency checking, structural review, and language refinement. All mathematical definitions, conventions, computations, references, claims, and conclusions reported in this article were reviewed and verified by the author.  The numerical and combinatorial results are generated independently by the deposited deterministic source code and machine-readable data rather than by the language model.  The generative AI system is not listed as an author and made no autonomous scientific or publication decisions.  The author takes full responsibility for the originality, accuracy, integrity, references, and conclusions of the submitted work.
...
% Tables placed after the appendices in accordance with journal instructions.
